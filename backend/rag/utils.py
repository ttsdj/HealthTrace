...
_milvus_manager = get_milvus_store("medical_qa")
...
def _finalize_retrieval(
    ...
    meta = {
        ...
        **candidate_config,
        ...
        "retrieval_attempts": retrieval_attempts or [{"mode": retrieval_mode, "status": "ok"}],
        "retrieval_degraded": bool(retrieval_attempts and len(retrieval_attempts) > 1),
...
def retrieve_documents(query: str, top_k: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        result = _finalize_retrieval(
            ...
            retrieval_attempts=attempts + [{"mode": "hybrid", "status": "ok", "raw_count": len(retrieved)}],
        )
        if result.get("docs"):
            return result
        attempts = result["meta"].get("retrieval_attempts", [])
        attempts[-1]["status"] = "empty_after_filter"
    ...
    try:
        ...
        result = _finalize_retrieval(
            ...
            candidate_k=candidate_k,
            candidate_config=candidate_config,
            retrieval_attempts=attempts + [{"mode": "dense", "status": "ok", "raw_count": len(retrieved)}],
            retrieval_failure_reason="hybrid_failed_or_empty",
        )
        if result.get("docs"):
            return result
        attempts = result["meta"].get("retrieval_attempts", [])
    ...
    try:
        ...
        result = _finalize_retrieval(
            ...
            candidate_k=candidate_k,
            candidate_config=candidate_config,
            retrieval_attempts=attempts + [{"mode": "sparse", "status": "ok", "raw_count": len(retrieved)}],
            retrieval_failure_reason="hybrid_and_dense_failed_or_empty",
        )
        if result.get("docs"):
            return result
        attempts = result["meta"].get("retrieval_attempts", [])
    ...
    return {
        ...
        "meta": {
            ...
            "rerank_model": RERANK_MODEL,
            "rerank_endpoint": _get_rerank_endpoint(),
            "rerank_error": "retrieve_failed_or_empty",
            "retrieval_mode": "no_results",
            "retrieval_attempts": attempts,
            "retrieval_degraded": len(attempts) > 1,
            "retrieval_failure_reason": "hybrid_dense_sparse_all_empty_or_failed",
            "retrieval_pipeline": "recall_merge_rerank",
            "candidate_k": candidate_k,
            **candidate_config,
            "retrieval_top_k": top_k,
