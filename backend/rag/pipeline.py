...
from backend.rag.conflict import detect_conflicts
from backend.rag.context_compression import compress_documents_for_context
from backend.rag.fusion import format_retrieval_context, fusion_mode
...
def _format_docs(docs: List[dict]) -> str:
    return format_retrieval_context(docs)


def _compress_docs_for_prompt(docs: List[dict], query: str) -> tuple[List[dict], dict]:
    return compress_documents_for_context(docs, query=query)


def _trace_quality_fields(docs: List[dict]) -> dict:
    conflict = detect_conflicts(docs)
    if conflict.get("conflict_detected"):
        emit_rag_step("⚖️", "检测到可能冲突的医学证据", conflict.get("conflict_summary", ""))
    return {
...
def retrieve_expanded(state: RAGState) -> RAGState:
    ...
    prompt_docs, compression_meta = _compress_docs_for_prompt(deduped, state["question"])
    context = _format_docs(prompt_docs)
