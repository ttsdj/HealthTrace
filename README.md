...
- 仓库提供统一的可复现评测工件 schema（`backend/evaluation/eval_artifact.py`）与 `scripts/evaluate_ragcare_full.py` 等脚本，会产出 `dataset_hash/config/per_query/statistical_definitions/command/commit` 的 `MANIFEST.json`。规则：指标数字只从真实跑数生成的工件读取；在真实结果工件产出前，上述各分数保持未验证。详见 [`docs/REPRODUCIBLE_EVALUATIONS.md`](docs/REPRODUCIBLE_EVALUATIONS.md)。
