<template>
  <aside class="retrieval-panel clinical-retrieval-panel">
    ...
    <div v-if="!trace" class="retrieval-empty">
      ...
      <p>发送问题后，这里会显示召回、重排、知识图谱、记忆命中、安全边界和本轮 RAGAS-lite 评估。</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="ragas-summary">
        <div class="panel-section-title">
          <span>RAGAS-lite 本轮评估</span>
          <small>{{ formatScore(trace.ragas_quality_score) }}</small>
        </div>
        <div class="ragas-score-grid">
          <div v-for="item in ragasScores" :key="item.key" class="ragas-score-card">
            <span>{{ item.label }}</span>
            <strong>{{ formatScore(item.value) }}</strong>
            <div class="score-bar"><i :style="{ width: scorePercent(item.value) }"></i></div>
          </div>
        </div>
        <p>{{ trace.ragas_evaluation_note || '实时无 reference 评估，用于观察本轮回答质量趋势。' }}</p>
      </div>
...
const ragasScores = computed(() => [
  { key: 'context', label: '上下文相关性', value: trace.value?.ragas_context_relevance },
  { key: 'faithfulness', label: '忠实度代理', value: trace.value?.ragas_faithfulness },
  { key: 'answer', label: '答案相关性', value: trace.value?.ragas_answer_relevance },
]);
const safetyLabel = computed(() => {
  if (!trace.value?.safety_guard_enabled) return '本轮未记录安全检测状态';
  if (trace.value.safety_notice) return trace.value.safety_notice;
  const flags = [];
  ...
  return flags.length ? flags.join(' / ') : '已完成安全检测，未触发高风险/剂量/隐私规则';
});
...
const scorePercent = (value?: number | null) =>
  `${Math.max(4, Math.round(Math.min(Math.max(Number(value ?? 0), 0), 1) * 100))}%`;
