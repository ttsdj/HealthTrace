      ...
      <section v-if="store.notifications.length" class="task-section">
        ...
        <div class="notification-list">
          <article v-for="notice in store.notifications.slice(0, 6)" :key="notice.notification_id" :class="['notification-row', notice.status]">
            ...
            <div><small>{{ formatDateTime(notice.created_at) }} · {{ notice.notification_type }}</small><strong>{{ notice.title }}</strong><p>{{ notice.body }}</p></div>
...
const taskDraft = reactive<{ task_type: string; title: string; due_at: string; interval_seconds: number | null }>({ task_type: 'reminder', title: '', due_at: '', interval_seconds: null });
...
const createTask = async () => {
  ...
  try {
    await store.createTask({ task_type: taskDraft.task_type, title: taskDraft.title, due_at: dueAt.toISOString(), timezone: 'Asia/Shanghai', interval_seconds: taskDraft.interval_seconds, payload: {}, idempotency_key: `ui-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` });
    taskDraft.task_type = 'reminder'; taskDraft.title = ''; taskDraft.due_at = ''; taskDraft.interval_seconds = null;
  } catch (error: any) { alert(error.message); }
};
