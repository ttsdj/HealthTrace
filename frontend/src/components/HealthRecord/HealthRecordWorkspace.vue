<template>
  <section class="health-record-workspace">
    ...
    <div v-else class="record-pane">
      <div class="pane-toolbar"><div><h2>长期健康任务</h2><p>提醒先创建为草稿，需再次确认后才会激活。</p></div></div>
...
const taskDraft = reactive<{ title: string; due_at: string; interval_seconds: number | null }>({ title: '', due_at: '', interval_seconds: null });
...
const createTask = async () => {
  ...
  try {
    await store.createTask({ task_type: 'health_reminder', title: taskDraft.title, due_at: dueAt.toISOString(), timezone: 'Asia/Shanghai', interval_seconds: taskDraft.interval_seconds, payload: {}, idempotency_key: `ui-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` });
    taskDraft.title = ''; taskDraft.due_at = ''; taskDraft.interval_seconds = null;
  } catch (error: any) { alert(error.message); }
};
